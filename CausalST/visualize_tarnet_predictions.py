...
def plot_ate_heatmap(adata, treatment_effects, outfolder='/home/amonell/Desktop/spatial_perturb/2024_10_06_perturb4_no_baysor/CausalST/scripts/figures/tarnet/global'):
    ...
    sns.clustermap(ate_df.T, 
                  ...
                  center=0,
                  xticklabels=False)
    plt.title('Global Treatment Effects Heatmap (TARNet)')
    plt.savefig(os.path.join(outfolder, 'global_ate_heatmap_tarnet.pdf'), 
                bbox_inches='tight')
    plt.close()
    
    return ate_df

def plot_top_effects(adata, sgRNA, treatment_effects, n_genes=15, outfolder='/home/amonell/Desktop/spatial_perturb/2024_10_06_perturb4_no_baysor/CausalST/scripts/figures/tarnet/per_perturbation'):
    ...
    ATEs = pd.Series(
        np.mean(treatment_effects[sgRNA], axis=0),
        index=adata.var_names
    )
    ...
    top_genes = ATEs_sorted.head(n_genes).index.tolist()
    bottom_genes = ATEs_sorted.tail(n_genes).index.tolist()
    ...
    sc.pl.matrixplot(
        ...
        title=f'Treatment Effects by Cluster ({sgRNA}) - TARNet'
    )
    plt.savefig(os.path.join(outfolder, f'{sgRNA}_top_effects_tarnet.pdf'), 
                bbox_inches='tight')
    plt.close()
...
def plot_cluster_specific_effects(adata, treatment_effects, outfolder='/home/amonell/Desktop/spatial_perturb/2024_10_06_perturb4_no_baysor/CausalST/scripts/figures/tarnet/per_cluster'):
    ...
    for sgRNA, effects in treatment_effects.items():
        ...
        sns.clustermap(cluster_df,
                      cmap='coolwarm',
                      center=0)
        plt.title(f'Cluster-specific Treatment Effects ({sgRNA}) - TARNet')
        plt.savefig(os.path.join(outfolder, f'{sgRNA}_cluster_effects_tarnet.pdf'), 
                    bbox_inches='tight')
        plt.close()

def main():
    # Parameters
    path_to_adata = '/mnt/sata2/Analysis_Alex_2/perturb4_no_baysor/CausalST/final_object_tarnet_predictions.h5ad'
    base_outfolder = '/home/amonell/Desktop/spatial_perturb/2024_10_06_perturb4_no_baysor/CausalST/scripts/figures/tarnet'
